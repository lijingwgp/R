...
### Question 4 ###
##################
## use the knn.reg() function to build a knn regression model to predict the applications
## variable. you may choose to use all or only some of these predictors in the data set
## your answer will simply be the value of k that you think best determines the level 
## of flexibility for the situation. just provide it as a comment for your answer
## if you choose to use only a subset of the variables as predictors
## also provide R statements used to omit the variables
app <- read.table("applications.train.csv", sep = ",", header = TRUE, stringsAsFactors = F)
...
trainset <- app[train,]
validateset <- app[validate,]
testset <- app[test,]

#c(4,6,8,10,11,15,17)
#-c(1,5,7,14)
#c(11,9,6,8,10,13,2,12,17,4,3,15)
##c(4,10,11,15,17)
train.x <- trainset[,c(4,6,8,10,11,15,17)]
train.y <- trainset$Applications
validate.x <- validateset[,c(4,6,8,10,11,15,17)]
...
for(k in numreps) {
  knn.pred <- knn.reg(train.x, validate.x, train.y, k = k)$pred
  validate.errors[k] <- mean((validate.y - knn.pred)^2)
}

min(validate.errors)
numreps[which.min(validate.errors)]
plot(NULL, NULL, type='n', xlim=c(0, 50), ylim=c(0,max(validate.errors)), xlab='Replication', 
     ylab='MSEs')
lines(seq(1,50), validate.errors, type='b', col=2, pch=16)
print("I choose column number 4,6,8,10,11,15,17 as my features to train this data set. K=2 when minimum MSE is 8955450")